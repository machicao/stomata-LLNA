 ...
 */
public class FeatExtracStomata_CethenanteLight extends Thread {

    private static final String outpath = "ResultadosRedesEstomatos/LightConditions (cthentante)/arffs/";
    private static final String path = "RedesDB/DatasetEstomatos/LightConditions (Ctenanthe)/";
    private final String rulesDB[];
    private static final double escala = .347;
    private static final double threI = 140.0 / escala;
    private static final double threF = 260.0 / escala;
    private static final double threSteps = 20.0 / escala;

    public FeatExtracStomata_CethenanteLight(String _rulesDB[]) {
        rulesDB = _rulesDB;
    }
...
//
        ...
        FeatExtracStomata_CethenanteLight thread1 = new FeatExtracStomata_CethenanteLight(rules1);
        FeatExtracStomata_CethenanteLight thread2 = new FeatExtracStomata_CethenanteLight(rules2);
        FeatExtracStomata_CethenanteLight thread3 = new FeatExtracStomata_CethenanteLight(rules3);
        FeatExtracStomata_CethenanteLight thread4 = new FeatExtracStomata_CethenanteLight(rules4);
        FeatExtracStomata_CethenanteLight thread5 = new FeatExtracStomata_CethenanteLight(rules5);
        FeatExtracStomata_CethenanteLight thread6 = new FeatExtracStomata_CethenanteLight(rules6);
        FeatExtracStomata_CethenanteLight thread7 = new FeatExtracStomata_CethenanteLight(rules7);
        FeatExtracStomata_CethenanteLight thread8 = new FeatExtracStomata_CethenanteLight(rules8);
        thread1.start();
        thread2.start();
        thread3.start();
        thread4.start();
...
//=============================================
        //MainFeatureExtractorCAGraphEstomatos_CethenanteLight.extractClassicalFeatures();
        //runWekaClassification("ResultadosRedesEstomatos/LightConditions (cthentante)/classicalarffs/");
    }
...
//            JWekaClassifier.runClassification(JWekaClassifier.NAIVEBAYES, arffFiles[i], directory + "/__" + arffName + "_NB.csv");
    ...
    public static void extractProposedFeatures(String rule) {
        ...
        String networkClasses[] = {"4H", "24H"};
        String networkAcronClasses[] = {"L4", "L24"};
        int T = 350;
    ...
    public static void extractClassicalFeatures() {
        ...
        for (int i = 0; i < networkClasses.length; i++) {
            ...
            for (int f = 0; f < fileName.length; f++) {
                ...
                //for (int Threshold = 50; Threshold <= 200; Threshold = Threshold + 10) {
                for (double Threshold = threI; Threshold <= threF; Threshold = Threshold + threSteps) {
                    Model oldcn = OpenNetworkFromCentroidsFiles.openCthenante(io.getDirectory() + fileName[f], Threshold);
        ...
        String arrfFile = "ResultadosRedesEstomatos/LightConditions (cthentante)/classicalarffs/feats_classic.arff";
